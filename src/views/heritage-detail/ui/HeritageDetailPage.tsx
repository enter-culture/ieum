    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="p-5 space-y-5">
        <div className="grid grid-cols-2 gap-4">
          <div><p className="text-white/50 text-xs mb-1">지정 구분</p><p className="text-sm">{heritage.category}</p></div>
          <div><p className="text-white/50 text-xs mb-1">지정 번호</p><p className="text-sm">{heritage.number}</p></div>
          <div><p className="text-white/50 text-xs mb-1">지정일</p><p className="text-sm">{heritage.designatedAt}</p></div>
          <div><p className="text-white/50 text-xs mb-1">전승 지역</p><p className="text-sm">{heritage.region}</p></div>
        </div>
        {heritage.holders.length > 0 && (
          <div><p className="text-white/50 text-xs mb-1">보유자</p><p className="text-sm">{heritage.holders.join(', ')}</p></div>
        )}
        <div>
